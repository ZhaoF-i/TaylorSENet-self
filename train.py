...
if __name__ == '__main__':
    ...
    criterion = RI_add_Mag(loss_type='mse')
    ...
    for epoch in range(start_epoch, config['MAX_EPOCH']):
        ...
        for i, batch_info in enumerate(tr_batch_dataloader.get_dataloader()):
            # features, labels = batch_info[0].cuda(), batch_info[2].cuda()
            mix_features, labels = batch_info[0], batch_info[2]

            # forward + backward + optimize
            optimizer.zero_grad()
            outputs = network(mix_features)
