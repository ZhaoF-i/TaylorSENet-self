...
import math
from components.NormSwitch import NormSwitch
...
class NET_Wrapper(nn.Module):
    def __init__(self, win_len, win_offset):
        ...
        self.TaylorSENet = TaylorSENet(cin=2,
                      k1=[1, 3],
                      k2=[2, 3],
                      c=64,
                      kd1=5,
    ...
    def forward(self, mix_input):
        mix_input = self.stft.transform(mix_input)
        out = self.TaylorSENet(mix_input.permute(0, 3, 1, 2))
        out = self.stft.inverse(out.permute(0, 2, 3, 1))
        return out
...
#                       is_param_share=False,
#                       is_encoder_share=False).cuda()
#     from utils1.utils import numParams
