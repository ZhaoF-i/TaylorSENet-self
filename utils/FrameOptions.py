...
def frames_data(noisy_speech, hop_size=160, use_window=True):
    ...
    noisy_shape = noisy_speech.shape
    ...
    if noisy_speech.size(1) % hop_size == 0:
        noisy_speech = noisy_speech[:, :noisy_speech.size(1) // hop_size * hop_size]
    else:
        tmp = torch.zeros((noisy_shape[0], (noisy_speech.size(1) // hop_size + 1) * hop_size))
        tmp[:, :noisy_shape[1]] = noisy_speech
        noisy_speech = tmp
    # left data
    ...
    frames = torch.cat([frame_speech[:, :-1], frame_speech2[:, :]], dim=-1).cuda()
    if use_window is True:
        frames = frames * hamming_win
    return frames


def frames_overlap(data, hop_size=160, use_window=True):
    hamming_win = torch.from_numpy((np.hamming(hop_size * 2)).astype(np.float32)).cuda().reshape(1, 1, hop_size * 2)
    if use_window is True:
    ...
    overlap_res[:, hop_size: -hop_size] = overlap_res[:, hop_size: -hop_size] / 2
    return overlap_res

def frames_overlap_cpu(data, hop_size=160, use_window=True):
