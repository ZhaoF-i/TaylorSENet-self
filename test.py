...
class Test(object):
    ...
    def forward(self, network):
        ...
        for i in range(tt_len):
            ...
            # """------------------------------------modify  area------------------------------------"""
            # if self.type == 'online':
            #     clean_name = tt_lst[i][:-7] + 'noise.wav'
            #     clean, _ = sf.read(self.inpath + '/' + clean_name)
            #     sf.write(self.outpath + tt_lst[i][:-len(self.suffix) - 1] + '_noise.wav', clean[:est.size], fs)
            # sf.write(self.outpath + tt_lst[i][:-len(self.suffix) - 1] + '_est.wav', est / alpha, fs)
            # sf.write(self.outpath + tt_lst[i][:-len(self.suffix) - 1] + '_mix.wav', mix[:est.size], fs)
...
if __name__ == '__main__':
    ...
    # outpath = '/data01/zhaofei/result/Predict_Noise_Attempt/PredictNextFrameNoise/'
    makedirs([outpath])
    ...
    if args.new_test:
        ...
        # set type and suffix for local test data
